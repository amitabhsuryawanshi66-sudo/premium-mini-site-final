...
| Impeccable | `.agents/skills/impeccable/SKILL.md` plus `npx impeccable` CLI | Installer found existing project-local skills and did not rewrite `.agents/skills/impeccable/*`; `npx impeccable` CLI/detector path is no longer detector-broken. | Frontend audit, shape, polish, UX writing, motion, visual critique. | Include current `npx impeccable` CLI, detector, help, or command output when relying on it as tooling. Do not claim repair from `SKILL.md` alone. |
...
- `npx impeccable skills install` was run and reported that skills were already installed; it was not run with `--force`, and the project-local `.agents/skills/impeccable` folder was unchanged by the installer.
- The `npx impeccable` CLI/help/detector path is verified with detector output. Do not claim the local skill folder was rewritten or repaired unless a future approved `--force` reinstall changes it.
